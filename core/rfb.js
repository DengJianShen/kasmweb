...
export default class RFB extends EventTargetMixin {
    constructor(target, touchInput, urlOrChannel, options) {
        ...
        // if (window.parent) {
        //     window.parent.extensionInsertText = this.sendShiftI
        //     window.parent.extensionReplaceText = this.sendShiftR
        // }

        this._canvas.onmousemove = debounce(() => {
            if (window.parent && window.parent.recycling) window.parent.recycling();
            if (window.parent && window.parent.tabAlive) window.parent.tabAlive();
        }, 500)
        
        this._canvas.onkeyup = debounce((e) => {
            if (window.parent && window.parent.recycling) window.parent.recycling();
            if (window.parent && window.parent.tabAlive) window.parent.tabAlive();
        }, 500)
    ...
    */
    ...
    // dengjianshen 自定义插入
    // sendShiftI() {
    //     console.log('sendShiftI')
    //     if (this._rfbConnectionState !== 'connected' || this._viewOnly) { return; }
    //     this.sendKey(KeyTable.XK_Shift_L, "ShiftLeft", true);
    //     this.sendKey(KeyTable.XK_I, "KeyI", true);
    //     this.sendKey(KeyTable.XK_Shift_L, "ShiftLeft", false);
    //     this.sendKey(KeyTable.XK_I, "KeyI", false);
    // }
    // dengjianshen 自定义替换
    // sendShiftR() {
    //     console.log('sendShiftR')
    //     if (this._rfbConnectionState !== 'connected' || this._viewOnly) { return; }
    //     this.sendKey(KeyTable.XK_Shift_L, "ShiftLeft", true);
    //     this.sendKey(KeyTable.XK_R, "KeyR", true);
    //     this.sendKey(KeyTable.XK_Shift_L, "ShiftLeft", false);
    //     this.sendKey(KeyTable.XK_R, "KeyR", false);
    // }
    ...
    _requestRemoteResize() {
        ...
        if (this.getQueryString().isGuest == true) {
            return
        }
    ...
    _sendAiContent(text) {
        return new Promise((resolve, reject) => {
            if (this._rfbConnectionState !== 'connected' || this._viewOnly) { resolve('error'); }
            // dengjianshen
            this.clipboardPasteFrom(text)
