...
const UI = {
    ...
    start() {
        localStorage.setItem('clipboard_up', true);
        localStorage.setItem('clipboard_down', true);
        localStorage.setItem('clipboard_seamless', true);
        localStorage.setItem('enable_webp', true);
        localStorage.setItem('resize', 'remote');
        localStorage.setItem('idle_disconnect', '60');
        localStorage.setItem('reconnect', 'true');
        localStorage.setItem('enable_ime', 'true')
        localStorage.setItem('video_quality', '5')
        ...
        window._sendAiContent = this.sendAiContent
    ...
    initSettings() {
        ...
        UI.initSetting('video_quality', 5);
        ...
        *****/
        if (WebUtil.isInsideKasmVDI()) {
            ...
            UI._sessionTimeoutInterval = setInterval(function() {
                ...
                const timeSinceLastActivityInS = (Date.now() - window.lastActiveAt) / 1000;
        ...
        document.addEventListener('keyup', function (event) {
            if (event.ctrlKey && event.keyCode === 86) {
                ...
                navigator.clipboard.readText().then(text => {
                    ...
                    if (window.parent && window.parent.saveCodeCount) {
                        window.parent.saveCodeCount(newText)
                    }
                }).catch(err => {});
            }
 ...
 * ------v------*/
    ...
    sendAiContent(text) {
        return new Promise(async (resolve, reject) => {
            UI.rfb._sendAiContent(text).then((status) => {
                resolve(status)
            })
        })
    },
